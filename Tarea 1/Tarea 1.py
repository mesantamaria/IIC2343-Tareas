...
def flotante_decimal(signo_s,significante,signo_e,exponente):
...
def determinar_exponente(numero_binario):
    ...
    parte_fraccionaria=numero_binario-int(numero_binario)
    print("fraccionaria",parte_fraccionaria)
    exponente=0
    exponente+=len(str(parte_entera))-1
    if parte_entera==0 and parte_fraccionaria!=0:   #lo segundo es para el caso especial del 0
...
data = open("pruebas.txt", "r") #args[1]
num_lineas=0
...
for linea in data:
    ...
    if len(significante)<=bits_s and len(exponente)<=bits_e:  #Ajustar segun cantidad de bits y obtener el nuevo decimal
        nuevo_decimal=flotante_decimal(signo_s, significante, signo_e,exponente)
